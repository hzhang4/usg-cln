...
# Call ancestor's init to set self.parent, extension, name and unit number
        ...
        if self.iclncb>0:          
            model.add_output_file(
                self.iclncb, fname=filenames[1],extension=extension[1], 
                binflag=True,package=ModflowCln._ftype()
            )
        self.iclnhd=unitnumber[2]
        #>0: File unit for CLN HDS; 0: No head output;
        if self.iclnhd>0:       
            model.add_output_file(
                self.iclnhd, fname=filenames[2],extension=extension[2],
                binflag=True,package=ModflowCln._ftype()
            )
        ...
        if self.iclndd>0:       
            model.add_output_file(
                self.iclndd, fname=filenames[3],extension=extension[3],
                binflag=True,package=ModflowCln._ftype()
            )
        ...
        if self.iclnib>0:       
            model.add_output_file(
                self.iclnib, fname=filenames[4],extension=extension[4],
                binflag=True,package=ModflowCln._ftype()
            )
        ...
        if self.iclncn>0:       
            model.add_output_file(
                self.iclncn, fname=filenames[5],extension=extension[5],
                binflag=True,package=ModflowCln._ftype()
            )
        ...
        if self.iclnmb>0:       
            model.add_output_file(
                self.iclnmb, fname=filenames[6],extension=extension[6],
                binflag=True,package=ModflowCln._ftype()
            )
        ...
        if self.ncln>0:   # Linear CLN segments 
            ...
            self.nndcln = Util2d(
                model, (self.ncln,), np.int32, nndcln, name="nndcln",
    ...
    def write_file(self, f=None):
        ...
        f_cln.write("%10d%10d%10d%10d%10d%10d%10d%10d" % (self.ncln,
            self.iclnnds,self.iclncb,self.iclnhd,self.iclndd,
            self.iclnib,self.nclngwc,self.nconduityp))
        ...
        if ext_unit_dict is not None:
            ...
            if iclnhd > 0:
                unitnumber[2], filenames[2] = model.get_ext_dict_attr(
                    ext_unit_dict, unit=iclnhd
                )
                model.add_pop_key_list(iclnhd)
            if iclndd > 0:
                unitnumber[3], filenames[3] = model.get_ext_dict_attr(
                    ext_unit_dict, unit=iclndd
                )
                model.add_pop_key_list(iclndd)
            if iclnib > 0:
                unitnumber[4], filenames[4] = model.get_ext_dict_attr(
                    ext_unit_dict, unit=iclnib
                )
                model.add_pop_key_list(iclnib)
            if iclncn > 0:
